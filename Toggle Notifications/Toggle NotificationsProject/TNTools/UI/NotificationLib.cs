...
namespace ToggleNotifications.TNTools.UI
{
    public class NotificationToggle
    {
        ...
        public void SetNotificationState(NotificationType notificationType, bool state)
        {
            notificationStates[notificationType] = state;
        }
        public void SetAllNotificationsState(bool state)
        {
            SetNotificationState("SolarPanelsIneffectiveMessage", state);
            SetNotificationState("VesselLeftCommunicationRangeMessage", state);
            SetNotificationState("VesselThrottleLockedDueToTimewarpingMessage", state);
            SetNotificationState("CannotPlaceManeuverNodeWhileOutOfFuelMessage", state);
            SetNotificationState("GamePauseToggledMessage", state);
        }

        public void SetSolarPanelState(bool state)
        {
            SetNotificationState(NotificationType.SolarPanelsIneffectiveMessage, state);
        }

        public void SetCommunicationRangeState(bool state)
        {
            SetNotificationState(NotificationType.VesselLeftCommunicationRangeMessage, state);
        }

        public void SetThrottleLockedWarpState(bool state)
        {
            SetNotificationState(NotificationType.VesselThrottleLockedDueToTimewarpingMessage, state);
        }
        public void SetManeuverNodeOutOfFuelState(bool state)
        {
            SetNotificationState(NotificationType.CannotPlaceManeuverNodeWhileOutOfFuelMessage, state);
        }
        public void SetGamePauseToggledState(bool state)
        {
            SetNotificationState(NotificationType.GamePauseToggledMessage, state);
        }
        // Notifications
